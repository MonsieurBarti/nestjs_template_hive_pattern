...
| [`CONVENTIONS.md`](CONVENTIONS.md)                           | Naming, file structure, code style  | Before writing code            |
| [`DATABASE.md`](DATABASE.md)                                 | Prisma setup and database guide     | Setting up database            |
| [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md)                   | Common issues and solutions         | When you encounter errors      |
| [`PATTERNS.md`](PATTERNS.md)                                 | Advanced architectural patterns     | Implementing complex features  |
...
2. **Every endpoint** must use `@CorrelationId()`
3. **Never** use `console.log` - use injected `BaseLogger`
4. **Commands** return `void`, **Queries** return data
5. **Always** use Zod with `@ZodSchema`, never `class-validator`
6. **All endpoints** must have Swagger decorators
...
│   ├── commands/              # Write (void)
│   └── queries/               # Read (data)
├── infrastructure/[entity]/   # Adapters
...
- **Writing data?** → Command (returns `void`)
- **Reading data?** → Query (returns data)

### Where does this logic go?

- **Business rules?** → Domain Entity
- **Orchestration?** → Command/Query Handler
